...
export default function Options({ info }) {
    ...
    const [editConversation, { isLoading }] = useEditConversationMutation();
    const { user } = useSelector((state) => state.auth) || {};
    const { email: userEmail } = user || {};

    const participantUser = userEmail === info.receiver.email ? info.sender : info.receiver;
    ...
    const handleSubmit = (e) => {
        ...
        editConversation({
            ...
            sender: userEmail,
            data: {
                participants: `${userEmail}-${participantUser?.email}`,
